...
class Window:
    ...
    def print(self, pixels: List[Pixel]):
        table = [[Pixel(Color(0)) for _ in range(self.width)] for _ in range(self.height)]
    ...
    # TODO: allow center and left-alignment, adjust repeat functionality accordingly
    def render_text(self, text: str, font: Font, repeat = True, separator = ' ', inverse = False) -> List[Pixel]:
        text_rv = text[::-1] + separator[::-1] # Reverse the text, for a right-aligned layout
        blank_pixel = Pixel(Color(0) if not inverse else Color(4))
            ...
            for c in text_rv:
                ...
                for x in range(glyph.width - 1, -1, -1):
                    ...
                    for pixel in glyph.get_col(x):
                        pixels[i] = pixel if not inverse else Pixel(Color(4 - pixel.color.value))
                        i += 1
        ...
        for j in range(self.padding[3] * self.height):
            pixels[len(pixels) - j - 1] = Pixel(Color(0))
